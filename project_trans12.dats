...
(
  let
    ...
    // fun get_func_name(prfx: string, env0: c2env): string =
    // (
    //   let
    //     val i = get_mark(env0) 
    //   in
    //     prfx + strptr2string(g0int2string(i))
    //   end
    // )
    fun trans12(tm: t1erm): (t2inslst, t2val) = trans12_term(env, tm)
  in
    case- t1m0.node() of
    ...
    (
      ...
      in
        ...
        //   let
        //     val tmp = t2tmp_new()
        //     val env_new = c2env_extend(env, t1v, T2Vtmp(tmp))
        //   in
        //     (mylist_nil(), T2Vtmp(tmp))
        //   end
        // )
      end
    )
    | T1Mlam(t1v(*lname*), t1p1(*ret type*), t1m1(*instructions*)) => 
    (
      let
        val new_env = c2env_extend(C2ENVmark(get_mark(env)+1, env), t1v, T2Vtmp(t1v.name()))
        val (ins1, val1) = trans12_term(new_env, t1m1)
      in
        (ins1, val1)
      end
    )
    | T1Mfix(t1v(*fname*), t1p1(*ret type*), t1m1(*instructions*)) => 
    (
      let
        fun pull_lam_vals(t1m2): t2valist =
        (
          case- t1m2.node() of
          | T1Mlam(t1v, _, t1) => mylist_cons(T2Vtmp(t1v.name()), pull_lam_vals(t1)) 
          | _ => mylist_nil()
        )
        val tmp = t2tmp_new()
        val fname = t1v.name()
        val new_env = C2ENVmark(get_mark(env)+1,c2env_extend( env, t1v, T2Vfun(fname)))
        val (ins1, val1) = trans12_term(new_env, t1m1)
    ...
    (
      let
        ...
        // val inss = T2INSclo(tmp, tmp, mylist_cons(val1, mylist_nil()))
      in
        (inssdcl+inss1(*+inss*), T2Vtmp(tmp))
      end
    )
    | T1Mopr1(opr, t1m1) => 
    (
    ...
    | T1Mcond(t1cond(*test*), t1m1(*then*), t1m2(*else*)) => 
    (
      let
        val tmp = t2tmp_new()
        ...
        val ins3 = T2INScond(tmp, condval, insthen+insthenmove, inselse+inselsemove)
      in
...
// where
// {
//   val () = println!("trans12_term: t1m0 = ", t1m0, " with current env as: ", env)
// }
(* ****** ****** *)
...
(
  ...
  in
    ...
    (
      let
        val (tins, t2v0) = trans12_term(env, t1erm_make2(tv0.type(), T1Mfix(tv0, tv0.type(), t1m0)))
      in
...
local
  ...
  (
    ...
    | C2ENVcons(t1v, _, tl) => mylist_cons(t1v.name(), convert_dcl_to_string(tl))
  )
in
