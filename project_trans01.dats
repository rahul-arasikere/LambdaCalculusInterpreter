...
extern fun trans01_type: type0 -> type1
(* ****** ****** *)

extern fun trans01_term(s0env, t0erm): t1erm
extern fun trans01_tdcl(s0env, t0dcl): t1dcl
...
local
  fun auxoprs(opr: string, t0mlst: mylist(t0erm), env0: s0env) : t1erm =
  (
    let
      ...
      (
        ...
        (
          case tl of
          | mylist_cons(hd1, tl1) => t1erm_opr2(opr, trans01_term(env0, hd), aux(tl))
  ...
  (
    ...
    | mylist_cons(hd, tl) => 
      (
        case tl of 
        | mylist_cons(hd1, tl1) => t1erm_tup(trans01_term(env0, hd), auxtup(tl, env0))
        | mylist_nil() => 
        (
          case hd of
          | T0Mtup _ => trans01_term(env0, hd) //tuples sometimes are parsed, dirty fix
  ...
  (
    ...
    | T0Ptup(fst, snd) => T1Ptup(type0_to_type1(fst), T1Ptup(type0_to_type1(fst), T1Pnil))
  )
  ...
  (
    ...
    | mylist_cons(hd, tl) => 
    (
      mylist_cons(trans01_tdcl(env, hd), trans01_tdcllist(env, tl))
    )
...
in
  ...
  (
    let
      fun trans01(tm: t0erm): t1erm = trans01_term(env0, tm)
    in
      case- tm0 of
      | T0Mnil _ => t1erm_nil()
      ...
      | T0Mlam(t0v, tp0arg(*arg*), t0m1, tp0res(*res*)) =>
      (
        let
          val tp1arg = 
          (
      ...
      (
        let
          ...
          (
            ...
            (
              case- t0mtlst of
              | mylist_cons(T0Mprj(t0m2, i1), _) => get_actual_element(t0m2, i+i1)
              | _ => exit(0)
            ) 
            | _ => (i, t0mtemp)
          )
      ...
      | T0Mcond(t0m1(*test*), t0m2(*then*), o0pt(*else*)) =>
      (
        let
        ...
        in
          ...
          | myoptn_nil() => t1erm_make2(t1p2.type(), T1Mcond(trans01(t0m1), t1p2, t1erm_nil()))
        end
      )
