...
extern
fun t1erm_interp1(t1m0 : t1erm, env0 : d1env) : value

extern
fun tdclist_interp1(t1dlc : t1dclist, env0 : d1env) : d1env

extern
fun tdclist_interp(t1dlc : t1dclist) : d1env
...
local
  ...
  // end of [aux_app]
    ...
    in
      case- v1 of
        | VALbtf (b1) => (if b1 then
          t1erm_interp1(t2, env0)
        else
          t1erm_interp1(t3, env0))
    end
...
in
  implement t1erm_interp1 (t1m0, env0) =
    (case- t1m0.node() of
      ...
      | T1Mfix (f0, _, t1m1) => (VALfix(f0, vlam)) where
      ...
      in
        case- tv0 of
          | VALtup (t1, t2) => t1
          | _ => let
            val () = println!( "t1mfst: Error reconciling tuple ,"
                             , t0
                             , "; got: "
                             , tv0
                             )
      ...
      in
        case- tv0 of
          | VALtup (t1, t2) => (t2)
          | _ => let
            val () = println!( "t1msnd: Error reconciling tuple: "
                             , t0
                             , "; got: "
                             , tv0
                             )
      ...
      | T1Mtup (t1, t2) => VALtup(t1erm_interp1( t1
                                               , env0
                                               ), t1erm_interp1(t2, env0))
  ...
  implement tdclist_interp (t1dlc) =
    tdclist_interp1(t1dlc, d1env_nil())
