...
local
  fun recursive_type_fetch(t1p0 : type1) : type1 =
    (case t1p0 of
      | T1Pext (t1pext0) => (case t1pext0.get() of
...
in
  implement t1erm_tinfer (t1) =
    (case t1.node() of
      ...
      | T1Mfst _ => auxfst(t1)
      | T1Msnd _ => auxsnd(t1)
      | T1Mtup (fst, snd) => (T1Ptup(t1erm_tinfer(fst), t1erm_tinfer(snd)))
      | T1Mapp (t1m0, t1m1) => (let
      ...
      | T1Mopr1 (opr, t1m0) => (let
        val t1p0 = t1erm_tinfer(t1m0)
      in
        case+ opr of
          | "print" => (case- t1p0 of
            | T1Pbas (nm1) when nm1 = "nil" => T1Pnil
            | T1Pbas (nm1) when nm1 = "int" => T1Pnil
            | T1Pbas (nm1) when nm1 = "bool" => T1Pnil
            | T1Pbas (nm1) when nm1 = "string" => T1Pnil)
          | _ => t1p0
      end)
      | T1Mopr2 (opr, t1m0, t1m1) => (let
        val t1p0 = t1erm_tinfer(t1m0)
      ...
      | T1Manno (t1m0, t1p0) => (let
        val t1p1 = t1erm_tinfer(t1m0)
        val- true = type1_unify(t1p0, t1p1)
      in
        t1p0
      end)
      | T1Mcond (t1m0, t1m1, t1m2) => (let
        val t1p0 = t1erm_tinfer(t1m0)
        val t1p1 = t1erm_tinfer(t1m1)
