...
implement fprint_t1erm (out, t1m) =
  (case- t1m.node() of
    ...
    | T1Mlam (x0, t1p1, t1m2) => fprint!( out
                                        , "T1Mlam("
                                        , x0
                                        , "; "
                                        , t1p1
    ...
    | T1Mfix (t1v, t1p1, t1m1) => fprint!( out
                                         , "T1Mfix("
                                         , t1v
                                         , "; "
                                         , t1p1
    ...
    | T1Mopr2 (opr, t1m1, t1m2) => fprint!( out
                                          , "T1Mopr2("
                                          , opr
                                          , ";"
                                          , t1m1
    ...
    | T1Mcond (t1m1, t1m2, t1m3) => fprint!( out
                                           , "T1Mcond("
                                           , t1m1
                                           , "; "
                                           , t1m2
...
in
  (* in-of-local *)
  implement t1erm_get_node (t1m) =
    t1m.t1erm_node
  
  implement t1erm_get_type (t1m) =
    t1m.t1erm_type
...
implement t1erm_int (i0) =
  t1erm_make2(T1Pint, T1Mint(i0))

implement t1erm_btf (b0) =
  t1erm_make2(T1Pbool, T1Mbtf(b0))

implement t1erm_str (s0) =
  t1erm_make2(T1Pstring, T1Mstr(s0))
...
implement t1erm_fst (t1m0) =
  (case- t1m0.type() of
    | T1Ptup (fst, snd) => (t1erm_make2(fst, T1Mfst(t1m0)))
    | T1Pext (ext) => (let
...
implement t1erm_snd (t1m0) =
  (case- t1m0.type() of
    | T1Ptup (fst, snd) => (t1erm_make2(snd, T1Msnd(t1m0)))
    | T1Pext (ext) => (let
      val fst = type1_new_ext()
      val snd = type1_new_ext()
      val () = ext.set(T1Ptup(fst, snd))
...
implement t1erm_opr1 (opr, t1m1) =
  (if opr = "print" then
    t1erm_make2(T1Pnil, T1Mopr1(opr, t1m1))
  else
    t1erm_make2(t1m1.type(), T1Mopr1(opr, t1m1)))
